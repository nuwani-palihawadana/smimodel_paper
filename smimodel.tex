...
% Authors
  ...
  \addresses{%
   ...
   \lfoot{\sf Palihawadana, Hyndman, Wang: 19 March 2024}
...
\todo[inline]{These need to be categorical, not numerical. But then there will be identifiability issues by including Month and Season. You only need Month.}
...
considered as predictors that may enter indices. The two calendar
...
\item
  \(\theta_{1}\) and \(\theta_{2}\) are the coefficients corresponding
  to the two linear predictor variables;
  and\todo{Categorical variables have more than one coefficient each}
\item
  \(\bm{\varepsilon}\) is the error term.
\end{itemize}
...
the GAIM. Note that here, the two categorical calendar variables were
excluded when estimating the PPR model.
...
Backward Elimination & 16 &  & 911.570 & 25.035\\
