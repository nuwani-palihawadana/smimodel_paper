...
out from the model achieveing variable selection.
...
\(k = 1, . . . , d\), and the vector of linear covariates denoted by
\(\bm{u}_{i}\).
...
The algorithm discussed in this paper apply to the SMI Model estimator
given below, where the squared error of the model
...
\(\bm{\alpha} = \left [\bm{\alpha}_{1}^{T}, \dots, \bm{\alpha}_{p}^{T} \right ]^{T}\),
\(\bm{g} = \{g_{1}, g_{2}, . . ., g_{p}\}\), \(\bm{1}(\cdot)\) is the
...
\end{aligned}
}\label{eq-smi-mip}\end{equation} where
...
the first step of CGAIM algorithm (\textbf{?@eq-4}), which is
