...
% Authors
  ...
  \addresses{%
   ...
   \lfoot{\sf Palihawadana, Hyndman, Wang: 18 March 2024}
...
we first need to provide an feasible initialisation for the index
...
contains \([ \bm{v}_{i1}, \dots, \bm{v}_{ip}]\), where
...
is also crucial to choose a suitable value for the big-\(M\) parameter,
as the strength of the MIP formulation depends on the choice of a good
...
\section{Benchmark Methods}\label{sec-benchmark}

Before demonstrating the SMI model on simulated and real data, we will
briefly introduce three popular benchmark methods that can be used in
comparisons.
...
\(\bm{\alpha}_{j} = ( \alpha_{j1}, \dots, \alpha_{jp} )^{T}, j = 1, \dots, q\)
...
\subsection{Group-wise Additive Index Model}\label{sec-gaim}
...
linear combination, making model interpretation challenging
...
(Section~\ref{sec-backward}). Next, a GAIM (Section~\ref{sec-gaim}) is
also presented. In the case of GAIM, maximum temperature lags, minimum
temperature lags, and vapour pressure lags are categorised into three
groups, where an index estimated for each group. Finally, we present the
forecasting errors of a PPR model. The number of indices in the PPR
model was taken as 3, matching the number of indices estimated by the
GAIM.
