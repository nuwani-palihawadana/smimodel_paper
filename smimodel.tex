...
\hypersetup{
  ...
  pdfauthor={Nuwani Palihawadana; Rob J Hyndman; Xiaoqian~Wang},
...
% Authors
  ...
  \addresses{%
   ...
   \lfoot{\sf Palihawadana, Hyndman, Wang: 15 April 2024}
...
\item
  ...
  \item
    ...
    \todo[inline]{Should it be: $\max(\mathrm{abs}(\text{PPR index coefficients}))$?}
  \item
    Set to zero all coefficients that fall below the calculated
    threshold. \todo[inline]{Should it be: ...whose absolute values...?}
  \item
...
\hspace{1em}$y_{2}$ & some $\bm{x}$ + all $\bm{z}$ & ($x_{0}, x_{1}, z_{2}$) ($x_{2}$) ($z_{3}, z_{4}$) & ($x_{0}, x_{1}, z_{4}$) ($x_{2}$) & ($x_{0}, x_{1}, x_{2}, z_{2}$) & ($x_{0}, x_{1}$) ($x_{2}, z_{2}, z_{3}$)\\
\addlinespace[0.6em]
...
model error) irrespective of the fact that in the ``all \(\bm{x}\) + all
\(\bm{z}\)'' case, ``Additive'' and ``Multiple'' options include a noise
variable. The suggests over-fitting when there is low signal-to-noise
...
level of humidity). The number of daily deaths is plotted against each
...
maximum temperature lags, minimum temperature lags, and vapour pressure
lags are intuitively categorised into three separate groups, with an
index estimated for each group.
...
the PPR model was taken as three, matching the number of indices
...
``PPR'' initialisation, \textbf{\emph{SMI Model (12, 0) - PPR}}, shows
...
\midrule
SMI Model (12, 0) - PPR & 47 & 5 & \textbf{80.334} & \textbf{6.841} & \textbf{99.926} & \textbf{7.643}\\
SMI Model (1, 0) - Additive & 47 & 45 & 151.408 & 9.816 & 190.880 & 11.107\\
SMI Model (12, 5) - Linear & 47 & 2 & 164.629 & 10.153 & 207.040 & 11.141\\
Backward Elimination & 36 &  & 148.387 & 9.808 & 162.608 & 10.034\\
...
Table~\ref{tbl-heat} also shows that \textbf{\emph{SMI Model (12, 0) -
...
have inferior forecasting performance compared to all benchmark models
...
We experimented with the number of pairs of fourier terms (\(K\)) that
best captures the day of the year effect, considering values from
\(K = 1\) to \(K = 10\). The SMI models fitted with one pair of fourier
terms (\(K = 1\)) resulted in the lowest test MSE. (i.e.~the day of the
year seasonal pattern follows a simple sine wave.)
...
comparison. \todo[inline]{Same question as in application 1.}
...
\(DOY\_C1\). Notice that both \textbf{\emph{SMI Model (1, 0) -
Additive}} and \textbf{\emph{SMI Model (1, 0) - Linear}} have
...
\todo[inline]{Just to double check, did you also use fourier terms to allow for seasonal patterns in all benchmark methods?}
...
Backward Elimination & 17 &  & \textbf{564.811} & \textbf{19.604}\\
...
\textbf{\emph{SMI Model (1, 0) - Additive}} and benchmark models are
plotted in Figure~\ref{fig-solarPred} for further comparison.
...
nonparametric additive index model with optimal predictor selection and
predictor grouping, which we refer to as Sparse Multiple Index (SMI)
Model. The SMI Modelling algorithm is an iterative procedure that is
developed based on mixed integer programming to solve an
