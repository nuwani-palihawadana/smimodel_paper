...
\textbf{Updating Index Coefficients}

We estimate the updated value of index coefficients
\(\bm{\alpha}^{new}\) through a MIQP:
...
all zeros (i.e.~zero index), such indices will be dropped out from the
model. Furthermore, similar to Section~\ref{sec-step1}, once the new
estimate \(\bm{\alpha}^{new}\) is obtained, we scale each estimated
...
\item
  ...
  \tightlist
  \item
    Number of indices in the iteration reaches \(q\); select the final
    fitted model as output.
  \item
...
iterations in step 3.3, and a convergence tolerance for coefficients of
\(0.001\) in step 3.5 in estimating all SMI Models.)
...
\cmidrule{3-6}
\multirow{-6}{*}{\raggedright\arraybackslash \hspace{1em}$y_{1}$} & \multirow{-2}{*}{\raggedright\arraybackslash some x + all z} & ($x_{0}, x_{1}, z_{2}, z_{4}$) & ($x_{0}, x_{1}$) ($z_{4}$) ($z_{1}$) & ($x_{0}, x_{1}, z_{2}, z_{4}$) & ($x_{0}, x_{1}, z_{2}, z_{4}$)\\
\cmidrule{1-6}
...
fitted with ``PPR'' initialisation. (Here, \(\lambda_{2} = 0\) means
that the \(\ell_{2}\)-penalty (i.e.ridge penalty) is omitted from the
model.) The estimated model, \textbf{\emph{SMI Model (12, 0) - PPR }},
did not drop any of the index variables, and resulted in 5 indices. The
best penalty parameter combination for the model estimated taking
``Additive'' model as the starting point was
\((\lambda_{0} = 1, \lambda_{2} = 0)\). The estimated SMI Model did not
...
initialisation selected \((\lambda_{0} = 12, \lambda_{2} = 5)\) as the
penalty parameter combination (\textbf{\emph{SMI Model (12, 5) - Linear
...
initialisation, \textbf{\emph{SMI Model (12, 0) - PPR }}, shows the best
forecasting performance on both the test sets, out of the three
different SMI Models estimated.

Furthermore, we present test set forecasting errors of three different
...
(Section~\ref{sec-backward}) is presented. Next, a GAIM
...
each of those groups. Finally, the forecasting errors of a projection
...
by the group-wise additive index model (GAIM).
...
According to Table~\ref{tbl-heat}, \textbf{\emph{SMI Model (12, 0) -
...
have inferior forecasting performance to all the benchmark models
considered.

The actual series of number of deaths and the predicted series from the
\textbf{\emph{SMI Model (12, 0) - PPR}} and the benchmark models on
...
Model algorithm is an iterative procedure that is developed based on
mixed integer programming to solve an \(\ell_{0}\)-regularised nonlinear
least squares problem.

The proposed SMI Model algorithm has a number of special features: 1) It
...
high-dimensional setting. Thus, the SMI Model algorithm is an objective
and a principled estimation and variable selection method that reduces
any subjectivity induced by different users. 3) It is capable of
...
indices = number of predictors (entering indices)). Hence, the SMI Model
algorithm is a more general estimation algorithm in the domain of
nonparametric additive models. 4) It provides the ability to include
...
of determining a generalised initialisation for the SMI Model algorithm
that will work in many different applications.
...
in which the proposed SMI Model algorithm performs better compared to
...
Therefore, we hope that the proposed SMI Model algorithm and possible
future extensions of it will be useful for different users in different
fields, with different levels of specific domain knowledge, as an
objective and a general forecasting tool. \newline
