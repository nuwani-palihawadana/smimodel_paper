...
As reviewed in the earlier section, the CGAIM algorithm proposed by
...
index model as in \textcite{Masselot2022}, which can be written as \[
  y_{i} = \beta_{0} + \sum_{j = 1}^{p}g_{j}(\bm{\alpha}_{j}^{T}\bm{x}_{ij}) + \sum_{k = 1}^{d}f_{k}(w_{ik}) + \bm{\theta}^{T}\bm{u}_{i} + \varepsilon_{i}, \quad i = 1, \dots, n,
\] where all the term are as defined in \textbf{?@sec-CGAIM}. However,
in our method, we allow the possibility for the index coefficient
...
matrix of partial derivatives of the right hand side of
