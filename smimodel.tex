...
First, if the low noise level is considered, for the first true model
\(y_{1}\), when either ``all x'' or ``all x + all z'' predictors are
provided, all four initialisation options estimate the correct index
...
``all x + all z'' cases, except with the ``Linear'' initialisation, the
...
structure. This suggests that it maight better to initialise the
