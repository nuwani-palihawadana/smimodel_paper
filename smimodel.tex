...
\item
  ...
  \item
    ...
    \todo[inline]{Should it be: $\max(\mathrm{abs}(\text{PPR index coefficients}))$? - \textbf{Corrected}}
  \item
    Set to zero all coefficients whose absolute values fall below the
    calculated threshold.
    \todo[inline]{Should it be: ...whose absolute values...? - \textbf{Corrected}}
  \item
...
\todo[inline]{Just to double check, did you also use Fourier terms to allow for seasonal patterns in all benchmark methods? - \textbf{Yes, used Fourier terms in GAIM and backward elimination, but not in PPR because it is impossible to include separate linear terms in stats::ppr().}}
