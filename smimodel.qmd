...
1.  **PPR: Projection Pursuit Regression Based Initialisation**
    ...
    c. Calculate a threshold $\tau = 0.1 \times \max(\text{PPR index coefficients})$. \todo[inline]{Should it be: $\max(\mathrm{abs}(\text{PPR index coefficients}))$?}
    d. Set to zero all coefficients that fall below the calculated threshold. \todo[inline]{Should it be: ...whose absolute values...?}
