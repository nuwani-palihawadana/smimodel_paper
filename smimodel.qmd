...
dataHeat <- readRDS("data/Heat_Corrected.rds")
dataHeat |>
...
Maximum temperature, minimum temperature, and vapour pressure (to represent the level of humidity) are considered as predictors in this empirical study. The number of daily deaths is plotted against each of these predictors in Figures [-@fig-Tmax]--[-@fig-Vp], respectively, where we can observe that the relationships between these predictors and the response are slightly nonlinear.
...
\todo[inline]{Have the model refitted using the data from 1990 to 2013?}
\todo[inline]{**Comment addressed**}
...
\todo[inline]{Is this the only method that has used the validation set?}
\todo[inline]{**Comment addressed**}
