...
We estimated the SMI Model for the mortality data using the "PPR" initialisation option, and we tuned the penalty parameters $\lambda_{0}$ and $\lambda_{2}$, over ranges of integers from 1 to 12, and 0 to 12 respectively, through a greedy search based on in-sample MSE. (Here, a greedy search is used instead of a grid search to reduce computational time.)

The penalty parameter combination $(\lambda_{0} = 11, \lambda_{2} = 12)$ yielded the model with the lowest error. The estimated model, ***SMI Model (11, 12)***, did not drop any of the index variables, and resulted in 7 indices. (We used $M = 10$, a convergence tolerance of $0.001$, $50$ maximum iterations, and a convergence tolerance of $0.001$ for coefficients in estimating the SMI Model.)

We evaluated the forecasting error of the model estimated using two different subsets of the original test set:
...
According to @tbl-heat, *SMI Model (11, 12)*, the nonparametric multiple index model estimated through the newly proposed algorithm, outperforms all the three benchmark models in terms of forecasting accuracy, for both *Test Set 1* and *Test Set 2*, 

The actual series of number of deaths and the predicted series from the *SMI Model (11, 12)* and the benchmark models on *Test Set 2* are plotted in @fig-heatPred for further comparison.

```{r}
#| label: fig-heatPred
#| fig-cap: Actual number of deaths vs. predicted number of deaths from "SMI Model (11, 12)" and benchmark models for Test Set 2
HeatPreds <- readr::read_csv("data/heat_predictions.csv")
HeatPreds %>%
  mutate(
    Date = as.POSIXct(Date)
  ) %>%
  ...
  geom_line(aes(x = Date, y = SMImodel, linetype = "SMI Model (11, 12)", colour = "SMI Model (11, 12)")) +
...
@fig-solar shows the time plot of daily solar intensity for the entire period. 

```{r}
#| label: fig-solar
#| fig-cap: Daily solar intensity in Amherst, Massachusetts - from February 2006 to February 2013
dataSolar <- readRDS("data/solar_daily_large.rds")
dataSolar %>%
  mutate(
    Date = as.POSIXct(Date)
  ) %>%
