...
% Authors
  ...
  \addresses{%
   ...
   \lfoot{\sf Palihawadana, Hyndman, Wang: 19 June 2024}
...
Based on several experiments, we propose four alternative methods for
...
\item
  ...
  using several different models as initialisations, optimising the SMI
  model for each of them, and picking the initial model that results in
  the lowest loss for the MIQP problem.
\end{enumerate}
...
(in parantheses) below the index sturctures are the test MSE values of
...
\caption{\label{tbl-simulation}Simulation experiment results - Index
structures and test MSEs of the estimated SMI models for each
combination of response, input predictors, and noise levels.}
...
have selected irrelevant variables for both \(y_{1}\) and \(y_{2}\). In
both the cases of ``all \(\bm{x}\)'' and ``all \(\bm{x}\) + all
\(\bm{z}\)'', all the models estimated for \(y_{1}\) (except for the
...
observed for \(y_{2}\), where, irrespective of the different index
...
produce higher test MSE values compared to those for \(y_{1}\).
...
Moreover, We used the commercial MIP solver \textbf{\emph{Gurobi}}
...
estimated with ``PPR'' initialisation resulted in the best forecasting
